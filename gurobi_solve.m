...
Ns=size(Tree.leaves,2);
nvar=sys.nx+sys.nu;
nc=size(sys.F{1},1);
nc_t=size(sys.Ft{1},1);
model.Q=sparse(nvar*non_leaf_node+Ns*sys.nx,nvar*non_leaf_node+Ns*sys.nx);
model.A=sparse((nc+sys.nx)*non_leaf_node+(nc_t+sys.nx)*Ns,nvar*non_leaf_node+Ns*sys.nx);
model.rhs=zeros((nc+sys.nx)*non_leaf_node+(nc_t+sys.nx)*Ns,1);

%model.varnames = cell(1);

Qsps=sparse(V.Q);
Rsps=sparse(V.R);
PolyConstsps=sparse([sys.F{1} sys.G{1}]);

%DynamicSps=sparse((non_leaf_node+K)*sys.nx,nvar*non_leaf_node+K*sys.nx);
krow=nc*non_leaf_node+nc_t*Ns;
...
model.rhs(1:nc*non_leaf_node)=kron(ones(non_leaf_node,1),sys.g{1});
...
model.sense=[repmat('<',nc*non_leaf_node+nc_t*Ns,1);repmat('=',(non_leaf_node+Ns)*sys.nx,1)];
model.obj=zeros(non_leaf_node*nvar+Ns*sys.nx,1);
model.lb=-inf*ones(non_leaf_node*nvar+Ns*sys.nx,1);
