...
% zoom-step 
%
% Syntax : [ alpha ] = WolfCndVersion3( Grad,Z,Y,d,ops)
%
% INPUT :       obj   :  Object of the algorithm
%               Grad  :  Gradient of the FBE
%               Y     :  Dual variable 
%               Z     :  Primal iterates at Y
...
%while(abs(curv_dir)>1e-10)    
    ...
    [Grad_new,Znew,details_new] =obj.grad_dual_envelopVersion2(Ynew,x0);
    % G_{\lambda}(Ynew)
    ...
    for j=1:Ns
        phi_c=phi_c+tree.prob(non_leaf+j)*Znew.X(:,non_leaf+j)'*V.Vf{j}*Znew.X(:,non_leaf+j)+...
            Ynew.yt{j}'*separ_var_new.yt{j}+0.5*lambda*norm(separ_var_new.yt{j})^2;
        new_curv_dir=new_curv_dir+Grad_new.yt{j}'*d.yt{j};
    end
    phi_c=-phi_c;
    
    phi_diff=phi_c-phi0; 
    %if(phi_diff>c1*alpha_c*curv_dir && abs(new_curv_dir)<=-c2*curv_dir)
    if(phi_diff>0 && abs(new_curv_dir)>-c2*curv_dir)
